#!/usr/bin/env python3
"""Reconstruct the v2 leave-django-out fusion λ-sweep. Does not overwrite v2 artifacts.

Fits a fresh FrozenHead(logreg) on the 269 non-django cached CLS vectors fused
with train-fold-scaled 12-d oracle AST metrics, using the same recipe as
text_runner._head_fold_metrics(features=fusion). Never loads a grouped
checkpoint (those were fit on all 500).
"""
...
# Published django-holdout fusion logistic (3-seed mean ± sample std) from
# data/outputs/runs/router-v2-oracle-metrics-ldo/results.json — do not overwrite.
_RECORDED_MEAN = 0.482111757973827
_RECORDED_STD = 0.01983550932195174
_RECORDED_SEEDS = {
    0: 0.4689505896402448,
    1: 0.47245857590685186,
    2: 0.5049261083743843,
}
_EXPECTED_ALWAYS_OPUS = 0.7056277056277057
_EXPECTED_ALWAYS_QWEN = 0.5800865800865801
_EXPECTED_ORACLE = 0.7445887445887446
_EXPECTED_IN_DIM = 780  # 768 CLS + 12 AST
_ENDPOINT_TOL = 1e-9
_MEAN_BAND_K = 2.0
...
def _unique_xy(curve: list[dict]) -> list[dict[str, float]]:
    out: list[dict[str, float]] = []
    for point in curve:
        xy = {
            "cheap": round(float(point["frac_cheap"]), 6),
            "rate": round(float(point["resolved_rate"]), 6),
        }
...
def _fit_ldo_fusion(examples, embeddings, metrics, config, seed: int):
    ...
    scaler = fit_metric_scaler([metrics[iid] for iid in train_ids])
    ...
    if len(x_train[0]) != _EXPECTED_IN_DIM:
        raise SystemExit(
            f"fusion in_dim={len(x_train[0])} expected {_EXPECTED_IN_DIM}"
        )
    head = FrozenHead(
        "logreg",
    ...
    payload["weighted_bce"] = _pos_weight(train) is not None
    payload["in_dim"] = len(x_train[0])
    return train, hold, scores, payload
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default="data/outputs/runs/v2-django-holdout-curve",
        help="New run dir. Will not write into router-v2-oracle-metrics*.",
    )
    parser.add_argument(
        "--curve-seed",
        type=int,
    ...
    parser.add_argument(
        "--seeds",
        default="0,1,2",
        help="Seeds to refit for the Route-AUC sanity gate (v2 used 0,1,2).",
    )
    ...
    forbidden = {
        (root / "data/outputs/runs/router-v2-oracle-metrics").resolve(),
        (root / "data/outputs/runs/router-v2-oracle-metrics-ldo").resolve(),
        (root / "data/outputs/runs/router-v2-oracle-metrics-only-ldo").resolve(),
    }
    if out_dir.resolve() in forbidden:
        raise SystemExit(f"refusing to write into v2 artifact dir {out_dir}")

    config = load_text_train_config(config_path=args.config, features="fusion")
    ...
    metric_path = default_metric_cache_path()
    cached_metrics = load_metric_cache(metric_path)
    missing_metrics = [iid for iid in ids if iid not in cached_metrics]
    if missing_metrics:
        raise SystemExit(
            f"oracle metric cache missing {len(missing_metrics)} ids; "
            "refusing to recompute AST"
        )
    metrics = {iid: cached_metrics[iid] for iid in ids}
    print(
        f"cls_cache={cache_path} n_vectors={len(embeddings)} "
        f"metric_cache={metric_path} n_metrics={len(metrics)} in_dim={_EXPECTED_IN_DIM}",
        flush=True,
    )
    ...
    for seed in seed_list:
        train, hold, scores, payload = _fit_ldo_fusion(
            examples, embeddings, metrics, config, seed
        )
        auc = float(payload["route_auc"])
        seed_aucs[seed] = auc
        ...
        print(
            ...
            f"in_dim={payload.get('in_dim')} route_auc={auc:.6f} "
            f"auroc={payload.get('auroc')}{recorded_note}",
            flush=True,
        )
    ...
    unique = _unique_xy(curve)
    out_dir.mkdir(parents=True, exist_ok=True)
    curve_json = out_dir / "v2_django_holdout_curve.json"
    curve_csv = out_dir / "v2_django_holdout_curve.csv"
    per_task = out_dir / "v2_django_holdout_per_task.csv"
    payload = {
        "arm": "fusion v2 leave-django-out curve reconstruction",
        ...
        "lambdas": len(curve),
        "in_dim": _EXPECTED_IN_DIM,
        "used_grouped_checkpoint": False,
        "scaler_fit_on": "train_fold_only",
        "endpoints": {
    ...
    write_run_manifest(
        ...
        {
            "run_id": "v2-django-holdout-curve",
    ...
    print(f"unique_xy n={len(unique)}", flush=True)
    if flags:
